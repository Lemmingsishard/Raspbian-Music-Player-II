...
def SelectSong(event):
    ...
    global PlayPauseState
    global CurrentTrackNumber
    PlayPauseState = 1 #pauses whatevers playing
    PlayPause() #actually pauses it
...
def NextTrack():
    global CurrentTrackNumber
    global PlayPauseState
    PlayPauseState = 1 #pauses the song
    PlayPause() #acutally pauses the song
    pygame.mixer.music.unload() #unloads the current song
...
file.add_command(label="Exit", command=quit) #closes the program its also safer to close this way
